...
do
    make clean
    make FILES="${Environment} ExpBusGridworld.cxx AgentDynaQ_psweep.cxx utils.c"
    b_str="${Environment%.*}"
    b_str="${b_str#*d}"
    ...
    do
        ...
        do
            ...
            do
                for theta in 0.05 0.1 0.2 0.4 0.8
                do
                    e_str="${epsilon//./}"
                    a_str="${alpha//./}"
                    t_str="${theta//./}"
                    value="value/DynaQ_value_e${e_str}_a${a_str}_n${n}_t${t_str}_bs${b_str}.dat"
                    steps="steps/DynaQ_steps_e${e_str}_a${a_str}_n${n}_t${t_str}_bs${b_str}.dat"
                    ./RL_Exp $epsilon $alpha $n $num_episodes $discount $theta $value $steps $cumsteps
                done
            done
        done
