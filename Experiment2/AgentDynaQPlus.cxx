...
//void update_Tau(int stp0, int atp0){
    //// Exploration Bonus +1 for all the state action pairs that haven't been visited
    //for (int i = 0; i < NUM_STATES;i++) {
        //for (int j=0; j < NUM_ACTIONS; j++) {
            //Tau[i][j] += 1;
        //}
    //}
    //// set the exploration bonus = 0 when the state action is visited. 
    //Tau[stp0][atp0] = 0;
//}
...
const action_t *agent_start(const observation_t *this_observation) {
    ...
    //initial values set to 0. Tau(s,a)=0
    Tau = (int **)malloc(sizeof(int *)*NUM_STATES);
    for (int i = 0; i < NUM_STATES;i++) {
        Tau[i] = (int *)malloc(sizeof(int) * NUM_ACTIONS);
        for (int j=0; j < NUM_ACTIONS; j++) {
            Tau[i][j] = 0;
        }
    }
...
const action_t *agent_step(double reward, const observation_t *this_observation) {
    ...
    max_Q = get_maxQ(stp1);
    Q[stp0][atp0] = Q[stp0][atp0] + alpha*(reward + kappa*sqrt(t-Tau[stp0][atp0]) + discount*max_Q - Q[stp0][atp0]) ;
    //printf("stp0:%d, atp0:%d, Tau:%d, exploration bonus:%g\n",stp0, atp0, Tau[stp0][atp0], kappa*sqrt(t-Tau[stp0][atp0]));

    // Update Tau after Q is updated with the visited stp0 and atp0
    Tau[stp0][atp0] = t;
...
void agent_end(double reward) {
    ...
    max_Q = get_maxQ(stp1);
    Q[stp0][atp0] = Q[stp0][atp0] + alpha*(reward + kappa*sqrt(t-Tau[stp0][atp0]) + discount*max_Q - Q[stp0][atp0]) ;
    //printf("stp0:%d, atp0:%d, Tau:%d, exploration bonus:%g, Q[stp0][atp0]:%g\n",stp0, atp0, Tau[stp0][atp0], kappa*sqrt(t-Tau[stp0][atp0]), Q[stp0][atp0]);
